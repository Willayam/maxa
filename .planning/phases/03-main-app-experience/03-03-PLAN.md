...
must_haves:
  truths:
    ...
    - "Component library unified (Button, Card, Text used consistently)"
  ...
  key_links:
    ...
    - from: "apps/mobile/app/quiz/summary.tsx"
      to: "apps/mobile/stores/coachStore.ts"
      via: "getMessage for quiz feedback"
      pattern: "useCoachStore.*getMessage"
---
  ...
  <action>
    ...
    **Add XP level progress:**
    ...
    <View style={styles.levelBadge}>
      <Text style={styles.levelText}>Nivå {level}</Text>
      <ProgressBar
    ...
    **Component implementation:**
    ...
    export function StreakMilestone({ visible, days, onDismiss }: StreakMilestoneProps) {
      ...
      const getMilestoneMessage = () => {
        if (days >= 30) return 'EN MÅNADS STREAK! 🏆';
      ...
      const getSubtitle = () => {
        if (days >= 30) return 'Du har tränat varje dag i en hel månad. Otroligt!';
        if (days >= 7) return 'Sju dagar i rad - du bygger en stark vana!';
        if (days >= 3) return 'Bra start! Tre dagar i rad visar på engagemang.';
        return 'Fortsätt så här!';
      };
...
<task type="auto">
  <name>Task 4: Audit Jag tab for component and design token consistency</name>
  <files>apps/mobile/app/(tabs)/jag.tsx</files>
  <action>
    After connecting Jag tab to stores, audit for consistent component and token usage.

    **Search for hardcoded colors (AUDIT-05):**
    ```bash
    grep -n "#[0-9a-fA-F]\{3,6\}" apps/mobile/app/\(tabs\)/jag.tsx
    grep -n "rgb\|rgba\|hsl" apps/mobile/app/\(tabs\)/jag.tsx
    ```

    Replace any found with design tokens from `@/constants/theme`.

    **Verify component library usage (AUDIT-04):**
    Ensure Jag tab uses shared UI components:
    ...
    **Fix any inconsistencies:**
    - Replace any raw react-native Text with themed Text component
    - Extract inline styles to StyleSheet
    - Use theme colors from Colors[colorScheme]
  </action>
  <verify>
    No hardcoded color hex codes: `grep "#[0-9a-fA-F]" apps/mobile/app/\(tabs\)/jag.tsx` returns empty
    All UI imports from @/components/ui/
    TypeScript compiles: `cd apps/mobile && npx tsc --noEmit`
  </verify>
  <done>
    Jag tab uses:
    - Shared UI components (Text, Button, Card, ProgressBar)
...
- All tabs use shared UI components consistently (AUDIT-04)
