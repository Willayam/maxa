...
for url in \
  "https://www.studera.nu/hogskoleprov/fpn/normeringstabeller-varen-2025/" \
  "https://www.studera.nu/hogskoleprov/fpn/normeringstabeller-hosten-2024/" \
  "https://www.studera.nu/hogskoleprov/fpn/normeringstabeller-varen-2024/"; do
...
for dir in varen-2025 hosten-2024 varen-2024; do
...
# Count total PDFs (should be 9)
ls -la apps/web/public/pdfs/{varen-2025,hosten-2024,varen-2024}/norm*.pdf 2>/dev/null | wc -l
# Expected: 9

# Verify tests.ts normering entry count
grep -c "normering" apps/web/src/data/tests.ts
# Should show increased count (was 3 for hosten-2025, now 12)
  </verify>
  <done>
9 normering PDFs downloaded and stored locally with verified file sizes (all >50KB). tests.ts updated with 9 new normering file entries for VT 2025, HT 2024, VT 2024.
  </done>
</task>
...
# 2. stdDev value
# 3. totalParticipants
# 4. First distribution row (hpScore 0.00)
# 5. Last distribution row (hpScore 2.00)
for slug in varen-2025 hosten-2024 varen-2024; do
  echo "=== Checking $slug ==="
...
# Start dev server and test the data loading
cd apps/web && node -e "
  // Test that the normering module can load the new data
  const fs = require('fs');
  const slugs = ['varen-2025', 'hosten-2024', 'varen-2024'];
...
# Build should succeed without errors
  ...
  <done>
3 new JSON files created (varen-2025.json, hosten-2024.json, varen-2024.json). Each file validated for structure and data accuracy (mean, stdDev, participants, distribution rows verified against source PDFs). Runtime verification confirms getNormeringData() loads data correctly. Build passes without errors.
  </done>
</task>
...
<verification>
1. PDF files exist in correct locations with verified sizes (>50KB each)
...
6. Runtime verification: getNormeringData() successfully loads new data
7. Build succeeds
8. Test pages display normering charts (manual verification: run dev server, visit /hogskoleprovet/varen-2025)
</verification>

<success_criteria>
- 9 normering PDFs downloaded and stored locally with verified sizes
...
- Runtime verification: getNormeringData() loads new data correctly
